...
%Macro CODI_APQ_COOCCUR;
	...
	%Let AGE_REQUEST = %Sysfunc(CATS(&AGE_20_24., &AGE_25_29., &AGE_30_34., &AGE_35_44., &AGE_45_54., &AGE_55_64.));
	%Let AGE_VALUES  = %STR(20 - 24,25 - 29,30 - 34,35 - 44,45 - 54,55 - 64);
	...
	%Else %If &GEO_GROUP. NE and &GEO_LIST. NE %Then %Do;
		...
		%Else %If %Sysfunc(UPCASE(&GEO_GROUP.)) = COUNTY %Then %Let GEOGRAPHY_FOOT  = %SYSFUNC(Compress(Geography: (&GEO_LIST.), %Str(%')));
	...
	%Selection_Fail(Var=Age, 	   Var_List=&AGE_20_24. &AGE_25_29. &AGE_30_34. &AGE_35_44. &AGE_45_54. &AGE_55_64.);
	...
	/*@Action: Year ***/
	%If not( 2000 <= &BEG_YEAR. <= 2030) or not(2000 <= &END_YEAR. <= 2030) %Then %Do;
		%Let FailureCode=2;
		%Goto Exit;
	%End;

	/*@Action: GEO_GROUP ***/
	%If &ALL_STATES.=N and not(&GEO_GROUP. in (STATE ZCTA3 COUNTY)) %Then %Do;
		%Let FailureCode=3;
		%Goto Exit;
	%End;
	...
	%If &ALL_STATES.=N %Then %Do;
		...
		%Do %While(&Scan_Geo. NE);
			%If (&GEO_GROUP.=STATE and %Length(&Scan_Geo.)^=4) or (&GEO_GROUP.=ZCTA3 and %Length(&Scan_Geo.)^=7) or (&GEO_GROUP.=COUNTY and %Length(&Scan_Geo.)^=7) %Then %Do;
				%Let FailureCode=4;
				%Goto Exit;
			%End;
	...
	Proc Sql NOPRINT;
		Create table Stacked_Years as
			Select Year, PATID, GEO3, Geography as Geography,
				Geography as STGEO3, State_Fips as State_Fips,
				Sex, &RACEVAR. as Race, Race_Imputed,
				WTCAT as Wgt_Cat Label = "Weight Category",
				AGEYR as Age Label = "Age, In Years", Age_Categories as Age_Categories,
				Pregnancy_flag, &CO_OCCURRING_COND_VAR. as co_occurring
					From SASIN.&FileIN_Name1.;
	Quit;
	/*@Action: Apply user selection criteria to EHR data file ***/
	/*@Action: Apply user selection of pregnancy flag ***/
	%If &INCLUDE_PREGNANCY.=N %Then %Do;
		Proc Sql NOPRINT;
			Create table User_Select as
				Select *
					From Stacked_Years
						Where &MASTER_LOGIC. and Wgt_Cat is not null and (Pregnancy_flag=0 or pregnancy_flag is null) and &BEG_YEAR.<=year<=&END_YEAR.
							Order by PATID, Year;
		Quit;
	%End;
	%Else %Do;
		Proc Sql NOPRINT;
			Create table User_Select as
				Select *
					From Stacked_Years
						Where &MASTER_LOGIC. and Wgt_Cat is not null and &BEG_YEAR.<=year<=&END_YEAR.
							Order by PATID, Year;
		Quit;
	%End;
	
	/*@Action: Execute sample size check if sample_check=Y ***/
	%If &sample_check.=Y %Then %Do;
		Proc Freq Data=User_Select noprint;
			Table Age_Categories/list missing Out=Geo_age_cat (Drop=Percent Rename=(Age_Categories=Values)); 
		...
		Data Geo_freq;
			...
			Set Geo_age_cat (in=a) Geo_race (in=b) Geo_sex (in=c);
			If a then Factor="Age Categories";
			Else if b then Factor="Race";
			Else if c then Factor="Sex";
		...
		Proc Print data=Geo_freq (drop=Count);
			title "Sample Size Checker Results";
		Run;
			title "";
	%End;
	...
	Proc sql Noprint;
		create table USER_W_Census as
			select a.*, b.ACS_POP_CNT, b.AGE_25_64_BACH_GRAD_GTR10PERC
	...
	Data Prepped_USER_File;
		Set Prepped_USER_File;
			...
			Orig_Educ_Raking  = AGE_25_64_BACH_GRAD_GTR10PERC; Educ_Raking  = AGE_25_64_BACH_GRAD_GTR10PERC;
	Run;
	...
	%Macro Proceed;
		...
		%If &Collapse_Pass.=1 %Then %Do;
			/*Action: Perform weighting using raking method on the fllowing SDOH: Geography(State+ZCTA3/COUNTY, Age, Race, Sex) ***/
			...
			%If &Covergence_Pass.=1 %Then %Do;
				...
				/*@Action: Create flags for each weight category ***/
				Data Weighted_USER_File;
					Set Weighted_USER_File;
						/*@Action: Create 0/1 flag for crude prevalence estimation ***/
						UndrWgt_Flg=(Wgt_Cat="(1) Underweight (BMI<18.5)");
						HlthyWgt_Flg=(Wgt_Cat="(2) Healthy Weight (18.5<=BMI<25)");
						OvrWgt_Flg=(Wgt_Cat="(3) Overweight (25<=BMI<30)");
						ObstyC123_Flg=(Wgt_Cat="(4) Obesity (Class 1) (30<=BMI<35)" or Wgt_Cat="(5) Obesity (Class 2) (35<=BMI<40)" or Wgt_Cat="(6) Obesity (Class 3) - Severe Obesity (BMI>=40)");
						ObstyC1_Flg=(Wgt_Cat="(4) Obesity (Class 1) (30<=BMI<35)");
						ObstyC2_Flg=(Wgt_Cat="(5) Obesity (Class 2) (35<=BMI<40)");
						ObstyC3_Flg=(Wgt_Cat="(6) Obesity (Class 3) - Severe Obesity (BMI>=40)");
						/*@Action: Create 0/1 flag for co-occuring condition crude prevalance estmation ***/
						%Do k=1 %To 3;
							cooccur&k.=(co_occurring=&k.);
						%End;
				Run;

				/*@Note: Prevalence Estimation and Output Routine ***/
				%Include "&PROGS_PQ.\&Estimate_Module_Cooccur..sas" / LRECL = 500;	
			%End;
			/*@Action: Raking has failed to converge ***/
			%Else %Do;
				%Generate_Prev_Report_CoOccur(Infile1=, Infile2=, Infile3=, Infile4=, Infile5=, Infile6=, Infile7=, Infile8=, Infile9=, Infile10=, Pass=N, FailCode=6);
			%End;
	  	%End;

		/*@Action: Collapsing fails to correct small weighting cell size ***/
		%Else %Do;
			%Generate_Prev_Report_CoOccur(Infile1=, Infile2=, Infile3=, Infile4=, Infile5=, Infile6=, Infile7=, Infile8=, Infile9=, Infile10=, Pass=N, FailCode=5);
		%End;
	%Mend;
