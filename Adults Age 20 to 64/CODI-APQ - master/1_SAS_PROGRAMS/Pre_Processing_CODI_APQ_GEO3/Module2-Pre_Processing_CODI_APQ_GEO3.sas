/*******************************************************************************************/
/***PROGRAM: Module2-Pre-Processing_CODI_APQ_ZCTA3.SAS					 				 ***/
/***VERSION: 1.0																		 ***/
...
Data People_with_Imputed_Hispanic;
	Set Imputed_Race_by_Cond;
		Where upcase(IMPUTE_RACE) contains "HISPA";
		Drop Impute_Race;
			Run;

/*@Action: add the patients who were imputed as Hispanic to the original file*/
Data People_with_Hispanic_Both;
	set People_with_Hispanic People_with_Imputed_Hispanic;
run;
...
Proc Sql NOPRINT;
	Select Count(Distinct Geography) Into :GEOG_CNT Trimmed From People_with_Hispanic_Both;
	Select Distinct Geography Into :GEOG_LIST Separated by " " From People_with_Hispanic_Both;
		Quit;
		...
		Proc Sql;
			Create table GEOG_&I. as
				Select *, Ranuni(&SEED.) as Randuni
					From People_with_Hispanic_Both
