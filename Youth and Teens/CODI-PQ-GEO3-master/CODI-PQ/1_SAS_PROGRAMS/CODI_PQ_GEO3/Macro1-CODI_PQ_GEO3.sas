/*******************************************************************************************/
/***PROGRAM: Macro1-CODI_PQ_GEO3.SAS									 				 ***/
/***VERSION: 1.0																		 ***/
...
%Macro Generate_ACS_Controls;
	...
	/*@Note: Join key is ST+ZCTA3 or ST+COUNTY							   ***/
	Proc Sql NOPRINT;
		Create table Census_ACS_Prep_Final as
			...
			select name into :ACS_Full_Var separated by " " from Dictionary.Columns where Libname="WORK" and Memname="CENSUS_ACS_PREP_FINAL" and Name not in ("State_FIPS","Geography","STGEO3","AGE_25_64_BACH_GRAD_GTR20PERC");
	Quit;
	...
	%Macro Reorient;
		...
		%DO %WHILE(&CURR. NE);
			%If "&Next."^="" %Then %Do;
				select Geography, State_FIPS, STGEO3, AGE_25_64_BACH_GRAD_GTR20PERC, "&Curr." as ACS_Var, &Curr. as ACS_Count from Census_ACS_Prep_Final union
			%End;
			%Else %Do;
				select Geography, State_FIPS, STGEO3, AGE_25_64_BACH_GRAD_GTR20PERC, "&Curr." as ACS_Var, &Curr. as ACS_Count from Census_ACS_Prep_Final
			%End;
	...
	Data ACS_Controls_Full_Universe;
		Set ACS_Universe_Controls;
			Wgt_Age_Categories=Put(Strip(Scan(ACS_Var, 1, "_")||" "||Scan(ACS_Var, 2, "_")||" "||Scan(ACS_Var, 3, "_")), $AGE_ACS.);
	...
	Proc Sql;
		Create table ACS_Age_Controls as
			Select Wgt_Age_Categories as AGE_Raking, Sum(ACS_Count) as mrgtotal From ACS_Controls_User_Universe Group by Wgt_Age_Categories;
